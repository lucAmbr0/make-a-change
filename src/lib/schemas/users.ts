...
export const userAuthenticationInput = zod.object({
  email: zod
    ...
    .max(256, "Invalid credentials")
    .toLowerCase()
    .trim(),
  password: zod
    .string({ message: "Password is required" })
    .min(8, "Invalid credentials")
    .max(72, "Invalid credentials")
    .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/, "Invalid credentials"),
});

export type userAuthenticationInput = zod.infer<typeof userAuthenticationInput>;

export const userIdSchema = zod.object({
  id: zod.number().int(),
});

export type userIdSchema = zod.infer<typeof userIdSchema>;

export const authenticatedUserSchema = zod.object({
    id: zod.number().int(),
    first_name: zod.string().max(32),
    last_name: zod.string().max(32),
    session_token: zod.string().min(10).max(500).optional(),
});

export type authenticatedUserSchema = zod.infer<typeof authenticatedUserSchema>;
...
export const createUserInput = zod.object({
  ...
  birth_date: zod.coerce
    .date({ message: "Birth date is required and must be a valid date" })
